...
<template>
    <div class="bg-white rounded-lg p-5 shadow-md">
        <!-- Button -->
        <div v-if="$can('index', 'Role')" class="flex justify-end mb-4">
        ...
        <!-- Form -->
        <vee-form :validation-schema="schema" @submit="StoreRole" validate-on-input>
            <!-- Role Name -->
            <div>
                <label for="role-name" class="block text-sm font-bold text-gray-700 mb-1.5">Role Name <span class="text-rose-400">*</span></label>
                <vee-field type="text" name="role_name" id="role-name" v-model="formData.role_name" class="block w-full p-2.5 rounded-md border border-gray-200 bg-gray-100 focus:outline-1 focus:outline-mainColor placeholder:text-sm" placeholder="Role Name" required />
            ...
            <div class="mt-4">
                ...
                <vee-field type="text" name="role_note" id="role-note" v-model="formData.role_note" class="block w-full p-2.5 rounded-md border border-gray-200 bg-gray-100 focus:outline-1 focus:outline-mainColor placeholder:text-sm" placeholder="Role Note" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-2">
                <div class="" v-for="(module, index) in moduleStore.modules" :key="module.id">
                    ...
                    <div class="flex items-center mb-4" v-for="(permission, index) in module.permissions" :key="permission.id">
                        <input :id="'permission-' + permission.id" type="checkbox" name="permissions[]" v-model="formData.permissions" :value="permission.id" class="w-4 h-4 text-white bg-gray-100 accent-mainColor border-gray-300 rounded focus:outline-0 focus:ring-2 focus:ring-mainColor" :checked="allPermission" />
