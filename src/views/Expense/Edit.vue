...
<template>
  <div class="bg-white rounded-lg p-5 shadow-md">
    ...
    <!-- Form -->
    <vee-form :validation-schema="schema" @submit="UpdateExpense" validate-on-input enctype="multipart/form-data">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div>
          ...
          <vee-field as="select" name="expense_category_id" id="expense-category" v-model="expenseStore.editFormData.expense_category_id" class="block w-full p-2.5 rounded-md border border-gray-200 bg-gray-100 focus:outline-1 focus:outline-mainColor cursor-pointer">
            <option value="">Select Category</option>
            <option v-for="category in expenseCategoryStore.categories" :key="category.id" :value="category.id">{{ category.category_name }}</option>
          </vee-field>
        ...
        <div>
          ...
          <vee-field as="select" name="staff_id" id="employee-name" v-model="expenseStore.editFormData.staff_id" class="block w-full p-2.5 rounded-md border border-gray-200 bg-gray-100 focus:outline-1 focus:outline-mainColor cursor-pointer">
            <option value="">Select Employee</option>
            <option v-for="employee in employeeStore.employees" :key="employee.id" :value="employee.id">{{ employee.name }}</option>
          </vee-field>
        ...
        <div>
          ...
          <vee-field type="number" name="amount" id="amount" v-model="expenseStore.editFormData.amount" class="block w-full p-2.5 rounded-md border border-gray-200 bg-gray-100 focus:outline-1 focus:outline-mainColor placeholder:text-sm" placeholder="Amount" min="0" />
      ...
      <div class="mt-5">
        ...
        <vee-field as="textarea" name="note" id="note" rows="3" v-model="expenseStore.editFormData.note" class="block w-full p-2.5 rounded-md border border-gray-200 bg-gray-100 focus:outline-1 focus:outline-mainColor placeholder:text-sm" placeholder="Write expense note" />
