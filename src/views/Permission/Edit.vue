...
const UpdatePermission = () => {
    permissionStore.updatePermission(permissionStore.editFormData, route.params.id);
}

/** Hooks */
onMounted(() => {
    permissionStore.getPermission(route.params.id);
...
<template>
    <div class="bg-white rounded-lg p-5 shadow-md">
        ...
        <!-- Form -->
        <vee-form :validation-schema="schema" @submit="UpdatePermission" validate-on-input>
            <!-- Module Name -->
            <div>
                <label for="module-name" class="block text-sm font-bold text-gray-700 mb-1.5">Module Name <span class="text-rose-400">*</span></label>
                <vee-field as="select" name="module_id" id="module-name" v-model="permissionStore.editFormData.module_id" class="block w-full p-2.5 rounded-md bg-gray-200 border border-gray-200 cursor-pointer focus:outline-1 focus:outline-mainColor">
            ...
            <div class="mt-4">
                ...
                <vee-field type="text" name="permission_name" id="permission-name" v-model="permissionStore.editFormData.permission_name" class="block w-full p-2.5 rounded-md bg-gray-200 border border-gray-200 focus:outline-1 focus:outline-mainColor placeholder:text-sm" placeholder="Permission Name" required />
